...
def obter_bundle_id(caminho_do_aplicativo):
    caminho_do_info_plist = os.path.join(caminho_do_aplicativo, "Contents", "Info.plist")
    
    if os.path.exists(caminho_do_info_plist):
        with open(caminho_do_info_plist, 'rb') as arquivo_plist:
            dados_do_plist = plistlib.load(arquivo_plist)
            bundle_id = dados_do_plist.get("CFBundleIdentifier")
            return bundle_id


def listar_aplicativos():
    diretorio_de_aplicativos = "/Applications"  #padrão Applications!

    if os.path.exists(diretorio_de_aplicativos):
        lista_de_aplicativos = []
        for idx, nome_do_aplicativo in enumerate(os.listdir(diretorio_de_aplicativos), start=1):
            caminho_do_aplicativo = os.path.join(diretorio_de_aplicativos, nome_do_aplicativo)
        ...
        for idx, nome_do_aplicativo, bundle_id in lista_de_aplicativos:
            print(f"{idx}. Aplicativo: {nome_do_aplicativo} - Bundle ID: {bundle_id}")

        entrada_do_usuario = input("Escolha o número do aplicativo ou digite um Bundle ID: ")
        ...
        if aplicativo_selecionado:
            ...
            escolhas = input("Digite o número do serviço que deseja conceder: ")
            escolhas = escolhas.split(',')
            escolhas = [int(escolha.strip()) for escolha in escolhas if escolha.strip().isdigit()]

            servicos_selecionados = [servicos_disponiveis[idx - 1] for idx in escolhas if 1 <= idx <= len(servicos_disponiveis)]
            ...
            print("Número de aplicativo ou Bundle ID inválido. Certifique-se de escolher um número na lista ou fornecer um Bundle ID válido.")
    else:
